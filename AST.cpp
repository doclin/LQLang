...
void AST::test()
{
    ...
    while(node != NULL)
    {
        ...
        {
            switch(static_cast<VarNode*>(node)->varType)
            {
                case TINT: 
                case TDOUBLE : IR.push_back(DALLOC); break;
                case TINTARR : IR.push_back(IAALLOC); IR.push_back(static_cast<VarNode*>(node)->arrLength); break;
                case TDOUBLEARR : IR.push_back(DAALLOC); IR.push_back(static_cast<VarNode*>(node)->arrLength); break;
            }
            node = node -> next;
        }
        else if(node->nodeType == FUNCNODE)
        {
            ...
            for(ASTNode* n=static_cast<FuncNode*>(node)->params; n!=NULL; n=n->next)
            {
